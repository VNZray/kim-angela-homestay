...
CREATE TABLE IF NOT EXISTS transaction (
    ...
    payment_method TEXT NOT NULL DEFAULT 'cash' CHECK (
        payment_method IN ('cash', 'gcash', 'bank_transfer', 'credit_card', 'other')
    ),
    transaction_status TEXT NOT NULL DEFAULT 'pending' CHECK (
        transaction_status IN ('pending', 'completed', 'failed', 'refunded')
    ),
...
-- STEP 6: Seed data
-- =====================================================
-- Seed data has been moved to a separate file: SEED_DATA.sql
-- Run SEED_DATA.sql after this migration to populate
-- default admin users, categories, business, rooms,
-- and amenities.
-- =====================================================
